...
### 2026-09-02（ステージングへ反映）

**概要:** 上の 36 件の修正を `staging` ブランチへ反映。本番（`main`）はまだ触っていない。動作確認の手順書を作り、確認待ち。

**気づいたこと:** 手元の `main` が 1 コミット古かった（`927980c` 2026-07-22「半月別CSVで実働ゼロ行を除外」）。
レビューはこの 1 件を含まない状態で行っていた。`origin/main` に rebase して取り込み直し、
`src/app/api/work-hours/export/route.ts` の `if (row.slotCount <= 0) continue;` が残っていることを確認済み。

**`origin/staging` の扱い:** staging には main に無い未リリース作業が 5 コミットあった
（`showCastSelector`、ダッシュボードの別実装、希望一覧まわりの手直し。2026-05-30〜06-02）。
混ぜると本番に出すものと別物を確認することになるため、**控えを取ってから入れ替えた**。

- 控え: `origin/staging-backup-2026-09-02`（`bd6f0c9`）… いつでも戻せる
- 同じ作業はローカルの `codex-staging-request-fix` にも残っている

**Git:**
- ブランチ `fix/review-2026-09-02`（`558fd6e`）を作成、`origin` へ push
- `origin/staging` = `558fd6e`（= `origin/main` の `927980c` ＋ 今回の修正）
- `origin/main` は未変更（`927980c` のまま）

**確かめたこと:** rebase 後に再度 `tsc` ○ / `eslint` 0 件 / `npm test` 7 本 95 件 ○ / `npm run build` ○
**確かめていないこと:** ステージングの画面。この環境のブラウザから当該ドメインへの接続が許可されず、
デプロイの成否も未確認。ログイン画面の説明文が「シフトの提出と確認」に変わっていれば新しいコード。

**次:** 手順書（18 項目）で管理者・キャストの確認 → 問題なければ `staging` → `main` の PR を作って本番へ。
⚠️ ステージングの DB が本番と別かは未確認。別でない場合は、書き込みを伴う 3・4・7・8・13・18 を飛ばす。

**Git:** コミット `558fd6e` / ブランチ `fix/review-2026-09-02`・`staging` / push 済み
