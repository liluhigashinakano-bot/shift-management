...
- **希望の登録・変更**は、基本的に **希望一覧** 画面で行います。
- 画面上部の **≡（メニュー）** から、他の画面へ移れることがあります。
- **確定シフト** では、自分に割り当てられたシフトを確認できます。
- **調整一覧** では、希望と確定の違いを確認できます。

> **画面のイメージ（キャスト・ヘッダー）**  
> 左上に「シフト管理」、右上に三本線のメニューボタン。期間の「前の期間」「次の期間」やプルダウンがある行もあります。

キャスト向けの画面の一例として、希望一覧は次のような見た目です（管理者が全員分を見る場合も同じ画面構成です）。

![希望一覧（キャスト利用時の例）](manual-images/05-requests.png)
