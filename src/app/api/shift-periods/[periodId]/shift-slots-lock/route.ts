...
export async function POST(
  ...
  if (role === "employee" && userStoreId && period.storeId !== userStoreId) {
    return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  }

  await prisma.shiftPeriod.update({
    where: { id: periodId },
    data: { shiftSlotsLocked: locked },
  });

  return NextResponse.json({ ok: true, shiftSlotsLocked: locked });
}
